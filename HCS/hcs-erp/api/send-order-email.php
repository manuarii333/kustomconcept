<?php
/* HCS ERP — Email confirmation commande (tunnels LP) */
...
if (!$o || empty($o['contact']['email'])) {
    ...
    echo json_encode(['error'=>'Donnees invalides ou email manquant']);
    exit;
}

$contact     = $o['contact'];
$delivery    = isset($o['delivery'])    ? $o['delivery']    : array();
$orderId     = isset($o['orderId'])     ? $o['orderId']     : (isset($o['order_id']) ? $o['order_id'] : '-');
$totalXpf    = (int)(isset($o['total']) ? $o['total'] : (isset($o['total_xpf']) ? $o['total_xpf'] : 0));
$productName = isset($o['productName']) ? $o['productName'] : (isset($o['cat_name']) ? $o['cat_name'] : '');

$clientEmail = filter_var($contact['email'], FILTER_VALIDATE_EMAIL);
if (!$clientEmail) { http_response_code(400); echo json_encode(['error'=>'Email invalide']); exit; }
...
$prodHtml = $productName
    ? '<tr><td style="padding:7px 10px;font-size:14px;color:#333">' . htmlspecialchars($productName, ENT_QUOTES, 'UTF-8') . '</td>'
      . '<td style="padding:7px 10px;text-align:right;color:#888;font-size:14px">' . number_format($totalXpf, 0, ',', ' ') . ' XPF</td></tr>'
    : '<tr><td colspan="2" style="padding:7px 10px;color:#888;font-size:13px">-</td></tr>';

$totalFmt = $totalXpf > 0 ? number_format($totalXpf, 0, ',', ' ') . ' XPF' : 'Devis a etablir';

$noteBlock = '';
if ($clientNote) {
    $noteBlock = '<tr><td style="padding:0 32px 20px">'
        . '<h2 style="font-size:13px;text-transform:uppercase;letter-spacing:1px;color:#888;margin:0 0 10px;border-bottom:1px solid #eee;padding-bottom:8px">Votre note</h2>'
        . '<p style="font-size:14px;color:#555;margin:0;font-style:italic">&ldquo;' . $clientNote . '&rdquo;</p>'
        . '</td></tr>';
}

$html = '<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Confirmation HCS</title></head>'
...
. '<h1 style="color:#f5ede0;margin:0;font-size:22px;font-weight:700">High Coffee Shirt</h1>'
. '<p style="color:#c8b89a;margin:6px 0 0;font-size:13px">Confirmation de votre commande</p>'
. '</td></tr>'
. '<tr><td style="padding:28px 32px 16px">'
. '<p style="font-size:16px;color:#1a0e07;margin:0 0 6px">Bonjour <strong>' . $clientName . '</strong>,</p>'
. '<p style="font-size:14px;color:#555;line-height:1.6;margin:0">Merci pour votre commande ! Votre paiement a ete accepte et votre commande est enregistree.</p>'
. '</td></tr>'
...
. '</div></td></tr>'
. '<tr><td style="padding:0 32px 20px">'
. '<h2 style="font-size:13px;text-transform:uppercase;letter-spacing:1px;color:#888;margin:0 0 10px;border-bottom:1px solid #eee;padding-bottom:8px">Detail commande</h2>'
...
. '<table width="100%"><tr>'
. '<td style="font-size:14px;color:#c8b89a;font-weight:700">Total paye</td>'
...
. '<p style="font-size:11px;color:#bbb;margin:0">Notre equipe vous contactera pour finaliser les details de votre commande.</p>'
. '</td></tr>'
...
$subjectB64     = base64_encode('Confirmation de commande - ' . $orderId);
